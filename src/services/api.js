...
window.ScaleNovaAPI = (function () {
  ...
  async function submitLead(formData, options = {}) {
    ...
    const submissionId = 'SN-D02-' + new Date().toISOString().slice(0, 10).replace(/-/g, '') + '-' + Math.floor(1000 + Math.random() * 9000);
    const instantSuccessResponse = {
      success: true,
      submissionId: submissionId,
      submission_id: submissionId,
      demoId: config.demoId || 'DEMO-02',
      leadType: payload.lead_type,
      message: 'Inquiry registered. Our industrial engineering team will review your specifications within 24 hours.'
    };
